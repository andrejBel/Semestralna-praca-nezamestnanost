...
namespace structures
{
	...
	inline TreeNode<T>* TreeNode<T>::deepCopy()
	{
		...
		for (int i = 0; i < degree(); i++)
		{
			...
			if (son)
			{
				...
				this->getSon(i)->setParent(this); // kvoli spravnej funkcnosti, vzhladom na to ze replace son resetuje otca
			}
		}
		return copy;
	}
	...
	inline TreeNode<T>* TreeNode<T>::getBrother(int brothersOrder)
	{
		...
		throw std::logic_error("Treenode has no parent");
	}
	...
	{
		return root_ == 0 ? 0 : root_->sizeOfSubtree();
	}
	...
	inline Iterator<T>& Tree<T>::TreeIterator::operator=(const Iterator<T>& other)
	{
		if (this != &other)
		{
			*this->path_ = *(dynamic_cast<const Tree<T>::TreeIterator&>(other).path_);
		}
		return *this;
	}
	...
	{
		if (this->path_->size() && dynamic_cast<const Tree<T>::TreeIterator&>(other).path_->size())
		{
			return this->path_->peek() != dynamic_cast<const Tree<T>::TreeIterator&>(other).path_->peek();
		}
		return this->path_->size() != dynamic_cast<const Tree<T>::TreeIterator&>(other).path_->size();
	}
