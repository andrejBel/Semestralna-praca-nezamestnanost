...
namespace structures
{
	...
	inline void ArrayList<T>::insert(const T & data, const int index)
	{
		...
		{
			enlarge();
			structures::Array<T>::copy((*array_), index, (*array_), index + 1, static_cast<int>(size_) - index);
			(*array_)[index] = data;
			size_++;
		}
	}
	...
	{
		DSRoutines::rangeCheckExcept(index, static_cast<int>(size_), "Index out of range");
		T temp = (*array_)[index];
	...
	inline void ArrayList<T>::enlarge()
	{
		...
		else 
		{
			...
			{
				Array<T>* newArray = new Array<T>(static_cast<int>(size_) * 2);
