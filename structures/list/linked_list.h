...
namespace structures
{
	...
	inline T LinkedList<T>::removeAt(const int index)
	{
		...
		else
		{
			...
			forRemove = previous->getNext();
			previous->setNext(forRemove->getNext());
			if (last_ == forRemove)
			{
				last_ = previous;
			}
	...
	{
		const LinkedList<T>::LinkedListIterator temp = dynamic_cast<const LinkedList<T>::LinkedListIterator &>(other);
