...
namespace structures
{
	...
	inline SequenceTable<K, T>& SequenceTable<K, T>::operator=(const SequenceTable<K, T>& other)
	{
		...
		{
			clear();
			for (TableItem<K, T>* temp : *other.list_)
			{
				list_->add(new TableItem<K, T>(*temp));
			}
		}
		return *this;
	}
	...
	inline T & SequenceTable<K, T>::operator[](const K key)
	{
		...
		throw std::out_of_range("Key is not in sequence table");
	}
	...
	template<typename K, typename T>
	inline void SequenceTable<K, T>::insert(const K & key, const T & data, bool check)
	{
		if (check) 
		{
			if (containsKey(key))
	...
	{
		return findTableItem(key) == 0 ? false : true;
	}
